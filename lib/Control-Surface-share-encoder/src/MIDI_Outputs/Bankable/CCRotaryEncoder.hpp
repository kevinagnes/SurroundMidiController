...
class BorrowedCCRotaryEncoder 
  ...
  public:
	...
	BorrowedCCRotaryEncoder(const OutputBankConfig &config, 
							Encoder &encoder,
	                        const MIDICNChannelAddress &address,
	                        int8_t speedMultiply = 1, uint8_t pulsesPerStep = 4,
	                        const RelativeCCSender &sender = {})
	        : BorrowedMIDIRotaryEncoder<RelativeCCSender>(config, encoder, address, 
							speedMultiply, pulsesPerStep, sender) {}
	};
